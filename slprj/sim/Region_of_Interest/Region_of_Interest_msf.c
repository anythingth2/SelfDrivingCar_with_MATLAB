...
#define S_FUNCTION_NAME Region_of_Interest_msf
#endif
...
MdlRefChildMdlRec childModels [ 1 ] = { "Region_of_Interest" ,
"Region_of_Interest" , 0 } ; const char * rt_GetMatSignalLoggingFileName (
...
3 , 1 ) ; if ( ! ssSetNumInputPorts ( S , 3 ) ) return ; if ( !
ssSetInputPortMatrixDimensions ( S , 0 , 240 , 320 ) ) return ;
...
, 0.0 ) ; if ( ! ssSetInputPortVectorDimension ( S , 1 , 261 ) ) return ;
...
; ssSetInputPortSampleTime ( S , 1 , 0.2 ) ; ssSetInputPortOffsetTime ( S , 1
, 0.0 ) ; ssSetOneBasedIndexInputPort ( S , 1 ) ; if ( !
ssSetInputPortVectorDimension ( S , 2 , 61 ) ) return ;
...
; ssSetInputPortSampleTime ( S , 2 , 0.2 ) ; ssSetInputPortOffsetTime ( S , 2
, 0.0 ) ; ssSetOneBasedIndexInputPort ( S , 2 ) ; if ( ! ssSetNumOutputPorts
( S , 1 ) ) return ; if ( ! ssSetOutputPortMatrixDimensions ( S , 0 , 61 ,
261 ) ) return ; ssSetOutputPortDimensionsMode ( S , 0 , FIXED_DIMS_MODE ) ;
...
mr_Region_of_Interest_RegisterSimStateChecksum ( S ) ; { static
...
mr_Region_of_Interest_MdlInfoRegFcn ( S , "Region_of_Interest" , & retVal ) ;
if ( ! retVal ) return ; }
#endif
#if SS_SFCN_FOR_SIM
if ( ssSetNumDWork ( S , 1 ) ) { int mdlrefDWTypeId ; ssRegMdlRefDWorkType (
S , & mdlrefDWTypeId ) ; if ( mdlrefDWTypeId == INVALID_DTYPE_ID ) return ;
if ( ! ssSetDataTypeSize ( S , mdlrefDWTypeId , sizeof ( ddfq4cerlym ) ) )
return ; ssSetDWorkDataType ( S , 0 , mdlrefDWTypeId ) ; ssSetDWorkWidth ( S
, 0 , 1 ) ; }
#else
if ( ! ssSetNumDWork ( S , 1 ) ) { return ; }
#endif
slmrSetDataTypeOverrideSettings ( S , 0 , 0 ) ; slmrSetRemoveDisableFunc ( S
...
static void mdlSetupRuntimeResources ( SimStruct * S ) { ddfq4cerlym * dw = (
ddfq4cerlym * ) ssGetDWork ( S , 0 ) ; void * sysRanPtr = ( NULL ) ; int
sysTid = 0 ; ssGetContextSysRanBCPtr ( S , & sysRanPtr ) ; ssGetContextSysTid
( S , & sysTid ) ; if ( sysTid == CONSTANT_TID ) { sysTid = 0 ; } oodkrbf2of
( S , ssGetSampleTimeTaskID ( S , 0 ) , & ( dw -> rtm ) , sysRanPtr , sysTid
...
* S , int_T tid ) { ddfq4cerlym * dw = ( ddfq4cerlym * ) ssGetDWork ( S , 0 )
; const uint8_T * i_ROOT_U0 = ( uint8_T * ) ssGetInputPortSignal ( S , 0 ) ;
const real_T * i_dwggkewior = ( real_T * ) ssGetInputPortSignal ( S , 1 ) ;
const real_T * i_ncxunwnatd = ( real_T * ) ssGetInputPortSignal ( S , 2 ) ;
uint8_T * o_ROOT_Y0 = ( uint8_T * ) ssGetOutputPortSignal ( S , 0 ) ; if (
tid != CONSTANT_TID && tid != PARAMETER_TUNING_TID ) { Region_of_Interest (
i_ROOT_U0 , i_dwggkewior , i_ncxunwnatd , o_ROOT_Y0 ) ; } } static void
...
dwFieldNames ) ; { mxArray * mdlrefDW = mr_Region_of_Interest_GetDWork (
ssGetDWork ( S , 0 ) ) ; mxSetFieldByNumber ( ss , 0 , 1 , mdlrefDW ) ; } {
mxArray * data = mr_Region_of_Interest_GetSimStateDisallowedBlocks ( ) ;
...
mr_Region_of_Interest_SetDWork ( ssGetDWork ( S , 0 ) , mxGetFieldByNumber (
